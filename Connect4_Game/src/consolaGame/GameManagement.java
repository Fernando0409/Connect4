...
public class GameManagement {
    ...
    private static boolean tokensVertical(String [][] tablero, String character){
        ...
        for(int i = getRows() - 1; i >= 0; i--){             //  6
            for (int j = 0; j < getColumns(); j++){     // 7c
                if(tablero[i][j].equals(character) && i-3 >= 0){
                    if(tablero[i-1][j].equals(character) && tablero[i-2][j].equals(character)
                                                        && tablero[i-3][j].equals(character)){
                        win = true;
                        break;
                    }
                }
    ...
    private static boolean tokensHorizontal(String [][] tablero, String character){
        boolean win = false;
        for(int i = getRows()-1; i >= 0; i--){      // 6
            for (int j = 0; j < getColumns(); j++){  // 7
                if(tablero[i][j].equals(character) && j+3 < getColumns()){
                    if (tablero[i][j+1].equals(character) && tablero[i][j+2].equals(character)
                                                         && tablero[i][j+3].equals(character)) {
                        win = true;
                        break;
                    }
                }
    ...
    private static boolean tokensDiagonal(String [][] tablero, String character){
        ...
        for(int i = getRows()-1; i >= 0; i--){
            for (int j = 0; j < getColumns(); j++){
                // Hacia el lado derecho ascendente/descendente
                if(tablero[i][j].equals(character) && i-3 >= 0 && j+3 < getColumns()){
                    if(tablero[i-1][j+1].equals(character) && tablero[i-2][j+2].equals(character)
                                                            && tablero[i-3][j+3].equals(character)){
                        win = true;
                        break;
                    }
                    // Hacia el lado izquierdo ascendente/descendente
                } else if(tablero[i][j].equals(character) && i -3 >= 0 && j-3 >= 0){
                    if(tablero[i-1][j-1].equals(character) && tablero[i-2][j-2].equals(character)
                                                            && tablero[i-3][j-3].equals(character)){
                        win = true;
                        break;
